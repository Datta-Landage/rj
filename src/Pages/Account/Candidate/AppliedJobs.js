...
const AppliedJobList = () => {
    ...
    async function showAllJobs() {
        try {
            const response = await fetch(`http://localhost:4000/AppliedJobList/${candidateId}`, {
                method: "GET",
                headers: {
    ...
    async function Apply(jobDataId) {
        try {
            ...
            const response = await fetch(`http://localhost:4000/jobApply/${candidateId}`, {
                method: "POST",
                headers: {
                    Accept: "application/json",
                    "content-Type": "application/json",
    ...
    async function Bookmark(jobDataId) {
        try {
            let data = { jobPostId: jobDataId }
            const response = await fetch(`http://localhost:4000/bookmarkJobPost/${candidateId}`, {
                method: "POST",
                headers: {
                    Accept: "application/json",
                    "content-Type": "application/json"
    ...
    return (
        <div>
                ...
                <div className="d-flex">
                    <div className={selectedJobs ? 'my-4' : "my-4 d-flex justify-content-start flex-wrap"} >
                        {filteredJobs.map((jobData, index) => (
                            <div
                                ...
                                <div className="d-flex justify-content-end  align-items-center">
                                    <h5
                                        onClick={() => Bookmark(jobData._id)}><big><i class="bi bi-bookmark-fill"></i></big></h5>
                                    <button className="btn-primary-cidblue mb-2 p-0 w-25"
                                        value={jobData._id}
                                        onClick={() => Apply(jobData._id)}
                                    >Apply</button>
                                </div>
                                <div className="d-flex align-items-center">
                                    <img className="rounded-circle bg-secondary m-2"
                            ...
                            <div>
                                {selectedJobs === jobData && (
                                    <div className="cid-box m-5 p-4">
                                        <div className="d-flex justify-content-end  align-items-center">
                                            <h5><big><i class="bi bi-bookmark-fill"></i></big></h5>
                                            <button className="btn-primary-cidblue mb-2 p-0 w-25"
                                                value={jobData._id}
                                                onClick={() => Apply(jobData._id)}
                                            >Apply</button>
